# Data preparaing script with execluding the 60m bands
import os
import sys
import json
import hashlib
...
# Sentinel-2 clip ranges per band number (1..13).
# You can keep these broad 0..10000 ranges or replace with your tighter per-band stats.
S2_CLIP = {
...
# === Exclude 60m bands ===
# Zero-based indices in the 13-band stack:
# B1=0 (60m), B9=9 (60m), B10=10 (60m)
EXCLUDE_S2_IDX = [0, 9, 10]
KEEP_S2_IDX = [i for i in range(13) if i not in EXCLUDE_S2_IDX]  # -> 10 bands
KEEP_S2_BAND_NUMS = [i + 1 for i in KEEP_S2_IDX]  # band numbers (1..13) kept

# make sure you adjust your paths!!
output_dir = "data-256_0.02_10000_60m_execluded"
...
P_TRAIN = 0.20
P_VAL   = 0.03
P_TEST  = 0.03  # <-- now test matches val
...
def s2_transform_selected_tanh(y_hwc):
    """
    Normalize and select only the kept S2 bands (exclude 60m bands).
    Input:  y_hwc shape (..., 13)
    Output: normalized selection shape (..., 10)
    """
    assert y_hwc.shape[-1] == 13, f"Expected 13 bands, got {y_hwc.shape[-1]}"
    H, W, _ = y_hwc.shape
    out = np.empty((H, W, len(KEEP_S2_IDX)), dtype=DTYPE)
    for j, k in enumerate(KEEP_S2_IDX):
        lo, hi = S2_CLIP[k + 1]  # band numbers are 1-based in S2_CLIP
        out[..., j] = to_tanh_range(y_hwc[..., k], lo, hi)
    return out
...
# S1 has 2 channels, S2 now has 10 channels after excluding 60m bands.
bytes_per_crop = (CROP_SIZE*CROP_SIZE*(2 + len(KEEP_S2_IDX))) * 4
...
for split in ["train", "val", "test"]:
    ...
    y_mm = open_memmap(y_path, mode="w+", dtype=DTYPE, shape=(n, CROP_SIZE, CROP_SIZE, len(KEEP_S2_IDX)))  # 10 channels
...
# Save ONLY the kept S2 bands' clip ranges, keyed by original band number (as strings)
S2_CLIP_KEPT = {str(bn): S2_CLIP[bn] for bn in KEEP_S2_BAND_NUMS}
with open(os.path.join(output_dir, "S2_clip.json"), "w") as f:
    json.dump(S2_CLIP_KEPT, f, indent=2)
...
def process_split(split):
    ...
    for s1f, s2f in tqdm(zip(s1_list, s2_list), total=len(s1_list), desc=f"{split} pairs"):
        ...
        for hh, ww in crop_grid(H, W, CROP_SIZE):
            ...
            # EXACT same normalization for S1; S2 normalized and reduced to kept bands
            Xn = s1_transform_db_tanh(X_patch)
            yn = s2_transform_selected_tanh(y_patch)  # -> (..., 10)
