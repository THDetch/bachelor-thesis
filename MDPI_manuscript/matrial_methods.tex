...
\paragraph{\textbf{SSIM}} The Structural Similarity Index (SSIM)~\cite{iqa_ssim} evaluates perceptual quality by comparing local luminance, contrast, and structure. Unlike pixel differences, it accounts for visual sensitivity to structural distortion:
\begin{equation}
\text{SSIM}(x,y) = \frac{(2\mu_x \mu_y + c_1)(2\sigma_{xy} + c_2)}{(\mu_x^2 + \mu_y^2 + c_1)(\sigma_x^2 + \sigma_y^2 + c_2)}
\end{equation}

\paragraph{\textbf{PSNR}} Peak Signal-to-Noise Ratio (PSNR) measures reconstruction quality via the ratio of maximum signal power to Mean Squared Error (MSE). It is a standard metric for pixel-level fidelity but correlates weakly with human perception~\cite{DiffCR}:
\begin{equation}
\text{PSNR}(x,y) = 10 \cdot \log_{10} \left( \frac{MAX^2}{\text{MSE}(x,y)} \right)
\end{equation}

\paragraph{\textbf{SAM}} Spectral Angle Mapper (SAM)~\cite{iqa_sam} evaluates spectral fidelity by treating pixels as \(n\)-dimensional vectors and measuring the angle between them. It is illumination-invariant, making it ideal for multispectral validation:
\begin{equation}
\text{SAM}(x,y) = \arccos \left( \frac{\langle x, y \rangle}{\|x\|_2 \cdot \|y\|_2} \right)
\end{equation}

\paragraph{\textbf{LPIPS}} Learned Perceptual Image Patch Similarity (LPIPS)~\cite{iqa_lpips} measures distance in deep feature space using a pretrained network. It captures high-level semantic similarity closer to human visual judgment than traditional metrics:
\begin{equation}
\text{LPIPS}(x,y) = \sum_{l} w_l \cdot \| f_l(x) - f_l(y) \|_2
\end{equation}
For inputs with $>3$ bands, LPIPS was computed on a representative RGB stack.
...
A summary of the evaluated aspects, advantages, and limitations of each metric is provided in Table~\ref{tab:evaluation_metrics}.
