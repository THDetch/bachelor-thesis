...
For the purpose of this thesis, which focuses on translating SAR data into multispectral optical imagery, only the Sentinel-1 and Sentinel-2 modalities are utilized. Representative examples of corresponding image pairs are shown in Figure~\ref{fig:sen12ms_pairs}. The MODIS component of the SEN12MS dataset was incorporated to provide land-cover annotations or auxiliary information supporting tasks such as land-cover mapping, semantic segmentation, and multi-sensor data fusion in deep learning research~\cite{sen12ms_2019}. However, since these data are not pertinent to the SAR-to-optical translation objective, they are excluded from the present study.
\begin{figure}[!htbp]
    \centering
...
A comparsion of these different datasets is provided in Table~\ref{tab:sen12_datasets}. In this thesis, however, the focus remains on the SEN12-MS dataset---since the focus is mainly on translating from SAR to optical and not directly to address cloud cover---
leveraging its multimodal SAR and multispectral imagery for the study of SAR-to-optical translation.
...
\end{equation}
where $\lambda$ balances realism and reconstruction accuracy. Following the original configuration proposed in~\cite{pix2pix_2018}, a weighting factor of \(\lambda = 100\) is commonly used in the literature~\cite{A_cGAN_fuse_sar_MS_CR, Mult_temp_S12_fus, quality_assessment_S2OT, CR_RS_GAN_s2o, trans_gan_CF, assessing_MT_cGANS_s2o_crop} and is therefore also adopted in the present thesis. In addition, supplementary loss components were incorporated, as they demonstrated improved training convergence and led to higher-quality reconstructions (see the ablation study on loss functions in Section~\ref{subsec:ablation_loss}).
...
\noindent
\textbf{Code Availability:} All source code developed for this thesis, including dataset preprocessing, model training, and evaluation scripts, will be made publicly available to support transparency and reproducibility. The implementation is based on the official Pix2Pix framework and has been adapted for multispectral SAR-to-optical image translation. The repository will be hosted at: \url{https://github.com/THDetch/bachelor-thesis}.
...
\end{itemize}
In the experiments, the weighting coefficients were set as follows: 
\(\lambda_{\text{L1}} = 100.0\) (as originally proposed in the Pix2Pix model and commonly used in the literature; see~\ref{sec:pix2pix_model}). 
Both \(\lambda_{\text{SSIM}}\) and \(\lambda_{\text{LPIPS}}\) were set to~$50$. 
These values were selected to maintain a balance between pixel-level accuracy and perceptual quality, while placing greater emphasis on the reconstruction process governed by \(\mathcal{L}_{\text{L1}}\). 
It is important to note that a comprehensive investigation of different values for \(\lambda_{\text{SSIM}}\) and \(\lambda_{\text{LPIPS}}\) was not performed.
...
Training was conducted for 150~epochs using a batch size of~16 and the Adam optimizer~\cite{adam_optimizer_2017} with a learning rate (\(\mathrm{LR}\)) of \(1\times10^{-4}\) and momentum parameters \((\beta_1, \beta_2) = (0.5, 0.999)\). To prevent overfitting and accelerate convergence, the generator’s learning rate was adaptively reduced using the \texttt{ReduceLROnPlateau} scheduler---an adaptive learning rate regulator that decreases the \(\mathrm{LR}\) by a specified factor (\(\mathrm{LR_{factor}}\)) when no improvement is observed in the validation loss over a predefined number of epochs, referred to as \emph{patience}~\cite{trans_breast_cancer_ReduceLROnPlateau}. The adjustment follows the rule 
\[
\mathrm{LR_{new}} = \mathrm{LR_{initial}} \times \mathrm{LR_{factor}}
\]
as described in~\cite{ReduceLRonPlateau_vs_OneCycleLR}. In this thesis, the learning rate was halved whenever the validation \(\mathrm{L1}\) loss stagnated for 10~epochs. The discriminator’s learning rate was kept constant throughout training to preserve stable adversarial dynamics, as reducing it was found to introduce additional instability during preliminary experiments.
...
Model performance was monitored using both quantitative validation metrics and qualitative visualizations. After each epoch, the validation set was evaluated to compute the $\mathrm{L1}$, SSIM, and LPIPS metrics. The model achieving the lowest validation $\mathrm{L1}$ loss was saved as the best-performing checkpoint. Additionally, intermediate qualitative samples were generated every ten epochs by translating a fixed subset of Sentinel-1 patches to their corresponding optical outputs, allowing visual inspection of the training progression. Samples are illustrated in Appendix~\ref{appendix:intermediate_visuals}
...
\cite{A_cGAN_fuse_sar_MS_CR, RS_Data_Fusion_GANs_sota, CR_RS_GAN_s2o, S2MS_GAN, c_guided_fus_s2ot, transfusion_cr, trans_gan_CF, cond_brownian, hvt_cgan, msf_gan} &
11 \\
...
\cite{A_cGAN_fuse_sar_MS_CR, CR_Advances_Review_ORS, RS_Data_Fusion_GANs_sota, CR_RS_GAN_s2o, c_guided_fus_s2ot} &
5 \\
...
\noindent
Overall, this combination of metrics provides a comprehensive assessment encompassing spatial fidelity, structural integrity, spectral accuracy, and perceptual quality. The evaluation scope of each metric, along with its advantages and limitations, is summarized again in Table~\ref{tab:evaluation_metrics}.
